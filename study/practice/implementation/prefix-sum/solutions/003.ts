// ================================================================
// study/implementation/prefix-sum - 003: 矩形区域の和
// (URL: study/practice/implementation/prefix-sum/problems/003/problem.md)
// TypeScript (Bun) Submission [using InputScanner]
// Get-Content study/practice/implementation/prefix-sum/problems/003/tests/sample.in | bun run study/practice/implementation/prefix-sum/solutions/003.ts
// ================================================================
...
function Main(inputText: string): void {
    ...
    const [H, W] = scanner.int(2);
    const A: number[][] = Array.from({ length: H! }, () => scanner.int(W!)!);
    const Q = scanner.int()!;
    ...
    for (let i = 0; i < Q; i++) {
        const R1 = scanner.int()!;
        const C1 = scanner.int()!;
        const R2 = scanner.int()!;
        const C2 = scanner.int()!;

        console.log(S[R2]![C2]! - S[R2]![C1 - 1]! - S[R1 - 1]![C2]! + S[R1 - 1]![C1 - 1]!);
    }
}
