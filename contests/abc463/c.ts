...
// Get-Content contests/abc463/tests/c.in | bun run contests/abc463/c.ts
// ================================================================
...
function Main(inputText: string): void {
    ...
    const N = scanner.int()!;
    type Takahashi = { H: number; L: number };
    const Takahashis: Takahashi[] = [];
    for (let i = 0; i < N; i++) {
        const [H, L] = scanner.int(2)!;
        Takahashis.push({ H: H!, L: L! });
    }
    const Q = scanner.int()!;
    const T = scanner.int(Q)!;

    const attention: Takahashi[] = [];
    let maxH = 0;
    Takahashis.reverse();
    Takahashis.forEach((takahashi) => {
        if (takahashi.H > maxH) {
            if (attention.at(-1)?.L === takahashi.L) {
                attention.pop();
            }
            attention.push(takahashi);
            maxH = takahashi.H;
        }
    });
    attention.reverse();
    ...
    T.forEach((Ti) => {
        const targetIndex = BinarySearch.upper_bound(
            attention,
            { H: 0, L: Ti },
            (a, b) => a.L - b.L,
        );
        const target = attention[targetIndex]!;
